...
@st.cache.data(ttl=60*60) # 1 hour cache
def cached_get_quote_data(symbol):
    return get_quote_data(symbol)

@st.cache.data(ttl = 24*60*60) # 24 hour cache
def cached_get_income_statement_data(symbol):
    return get_income_statement_data(symbol)
...
if run:
    ...
    with st.spinner(f"Fetching financial data for {symbol}..."):
        quote_data = cached_get_quote_data(symbol)
        income_statement_data = cached_get_income_statement_data(symbol)
